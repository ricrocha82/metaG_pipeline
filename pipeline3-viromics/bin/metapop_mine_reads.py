...
def read_one_range(command):
	...
	reader = subprocess.call(command)
...
def access_read_ranges(selected, threads, directory_base):
	...
	post_data = []
	range_key = 1
	range_base = os.path.normpath(directory_base + "/MetaPop/09.Linked_SNPs/" + "range_")
	for source in selected:
		...
		for s in selected[source]:
			...
			ranges.append(("samtools", "view", file, genome + ":" + str(begin)+"-"+str(end), "--output", range_base+str(range_key)+"_reads.txt", s[1],))
...
def do_mine_reads(output_directory, threads):
	snps = os.path.normpath(output_directory + "/MetaPop/07.Cleaned_SNPs/genic_snps.tsv")
	output_file = os.path.normpath(output_directory + "/MetaPop/09.Linked_SNPs/linked_snp_results.tsv")
